...
def main_sensor_fusion():
    """Main sensor fusion dengan perbaikan critical bugs"""
    
    print("=== SENSOR FUSION WITH BUG FIXES ===")
    ...
    try:
        ...
        while True:
            ...
            if gps_valid and not gps_initialized:
                ...
                # Leave other states as they are
            ...
            # Extract state - PERBAIKAN CRITICAL BUG!
            # Cek dimensi state vector dari EKF Anda
            if len(ekf.state) == 6:
                # Jika masih 6 dimensi
                est_x, est_y, est_heading, est_omega, est_velocity, est_accel = ekf.state
            elif len(ekf.state) == 4:
                # Jika sudah 4 dimensi
                est_x, est_y, est_heading, est_velocity = ekf.state
                est_omega = gyro  # Dari IMU langsung
                est_accel = accel  # Dari IMU langsung
            else:
