    ...
    while True:
        try:
            ...
            if len(GDT) >= 3:  # Pastikan ada minimal 3 GPS
                ...
                # Cek apakah jarak wajar (30cm ± 20cm tolerance)
                distances = [dist_12, dist_13, dist_23]
                if any(d > 1.0 for d in distances):  # Jika ada jarak > 1 meter
                    print("⚠️  WARNING: Jarak antar GPS terlalu besar! Mungkin koordinat referensi salah.")
                elif any(d < 0.1 for d in distances):  # Jika ada jarak < 10cm
                    print("⚠️  WARNING: Jarak antar GPS terlalu kecil! GPS mungkin terlalu dekat.")
                else:
                    print("✅ Jarak antar GPS dalam range wajar")
