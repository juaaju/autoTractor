...
def get_gps_positions(max_age=1.0):
        ...
        try:
            ...
            try:
                ...
                if position is not None:
                    ...
                    positions_latlon.append((lat, lon))
                    positions_xy.append(xy)
                    print(f"Menggunakan data lama untuk GPS {i+1}: {e}")
                else:
                    raise ValueError("Tidak ada data GPS valid")
            except Exception as e2:
                # Jika tidak ada data valid
                print(f"Tidak ada data untuk GPS {i+1}: {e2}")
...
try:
    ...
    while True:
        try:
            ...
            gps_latlon_data, GDT = get_gps_positions(max_age=2.0)
            ...
            if len(GDT) >= 3:  # Pastikan ada minimal 3 GPS
                ...
                print(f"GPS: {len(GDT)}/3 | Posisi: Est=[{pos_estimated[0]:.3f}, {pos_estimated[1]:.3f}] " +
