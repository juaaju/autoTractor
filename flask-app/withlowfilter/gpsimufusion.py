...
def quick_filter_demo():
    """Quick demo menunjukkan efek filtering"""
    print("=== Quick Filter Demo ===")
    
    # Simulate noisy data
    print("Simulating noisy sensor data...")
    
    from lowpass_filters import LowPassFilter
    import numpy as np
    import matplotlib.pyplot as plt
    
    # Generate test signal
    t = np.linspace(0, 2, 200)  # 2 seconds, 100Hz
    clean_signal = np.sin(2 * np.pi * 1 * t)  # 1Hz vehicle motion
    engine_vibration = 0.3 * np.sin(2 * np.pi * 15 * t)  # 15Hz engine vibration
    random_noise = 0.1 * np.random.normal(0, 1, len(t))
    noisy_signal = clean_signal + engine_vibration + random_noise
    
    # Apply filter
    filter_lowpass = LowPassFilter(3.0, 100.0, 'butterworth')  # 3Hz cutoff
    filtered_signal = [filter_lowpass.filter(val) for val in noisy_signal]
    
    # Calculate improvements
    clean_mse = np.mean((filtered_signal - clean_signal)**2)
    noisy_mse = np.mean((noisy_signal - clean_signal)**2)
    improvement = noisy_mse / clean_mse
    
    print(f"Filter improvement: {improvement:.1f}x better")
    print(f"Noise reduced by: {(1 - clean_mse/noisy_mse)*100:.1f}%")
    
    # Plot if matplotlib available
    try:
        plt.figure(figsize=(12, 6))
        plt.subplot(2, 1, 1)
        plt.plot(t, clean_signal, 'g-', label='True Vehicle Motion', linewidth=2)
        plt.plot(t, noisy_signal, 'r-', alpha=0.7, label='Raw Sensor (with engine vibration)')
        plt.legend()
        plt.title('Raw Sensor Data vs True Motion')
        plt.grid(True)
        
        plt.subplot(2, 1, 2)
        plt.plot(t, clean_signal, 'g-', label='True Vehicle Motion', linewidth=2)
        plt.plot(t, filtered_signal, 'b-', label='Filtered Sensor Data', linewidth=2)
        plt.legend()
        plt.title('Filtered Data - Engine Vibration Removed')
        plt.grid(True)
        plt.xlabel('Time (s)')
