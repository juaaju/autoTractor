...
# Kelas untuk mengelola map matching
class PathTracker:
    def __init__(self):
        # Buat jalur dari data yang diberikan
        self.path = generate_path_coordinates()
        print(f"Jalur dibuat dengan {len(self.path)} titik")
        
        # Inisialisasi MapMatcher
        self.map_matcher = MapMatcher(self.path)
        
        # Hasil terakhir dari map matching
        self.last_match_result = None
        
    def update(self, gps_lat, gps_lng) -> Dict[str, Any]:
        """
        Melakukan map matching untuk posisi GPS saat ini
        ...
        result = self.map_matcher.match_point([gps_lat, gps_lng])
        self.last_match_result = result
        
        return result
        
    def get_last_match(self) -> Optional[Dict[str, Any]]:
        """Mendapatkan hasil map matching terakhir"""
...
class DataCollector:
    def __init__(self, log_to_csv=True):
        ...
        # Initialize PathTracker for map matching
        ...
        if self.log_to_csv:
            ...
            # Tulis header - tambahkan kolom untuk map matching
            self.csv_writer.writerow([
                ...
                'map_matched_lat', 'map_matched_lng', 'distance_to_path', 'progress'
            ])
    ...
    def calibrate_imu(self, samples=1000, delay=0.01):
        """
    ...
    def _log_to_csv(self, sensor_data, odom_data, ekf_data, map_match_result):
        """
        ...
        Args:
            ...
            map_match_result: Hasil map matching
        """
        try:
            ...
            # Data map matching
            if map_match_result:
                matched_lat, matched_lng = map_match_result['matched_point']
                distance = map_match_result['distance_to_path']
                progress = map_match_result['progress']
            else:
                matched_lat, matched_lng = None, None
                distance, progress = None, None
            ...
            self.csv_writer.writerow([
                ...
                matched_lat, matched_lng, distance, progress
            ])
    ...
    def get_map_match_result(self) -> Optional[Dict[str, Any]]:
        """
        Mendapatkan hasil map matching terakhir
        
        Returns:
            Dict atau None: Hasil map matching jika tersedia
        """
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            match_result = collector.get_map_match_result()
            ...
            # Tampilkan informasi Map Matching
            if match_result:
                matched_lat, matched_lng = match_result['matched_point']
                distance = match_result['distance_to_path']
                progress = match_result['progress'] * 100  # Persentase
                is_on_path = match_result['is_on_path']
                
                print("=" * 50)
                print(f"MAP MATCHING:")
                print(f"Matched  : Lat={matched_lat}, Lng={matched_lng}")
                print(f"Distance : {distance:.2f} meter dari jalur")
                print(f"Progress : {progress:.1f}% dari jalur")
                print(f"On Path  : {'Ya' if is_on_path else 'Tidak'}")
